...
import postsPageInitialPaths483bfResource from '../../resources/posts-page-initial-paths-483bf'
import postsPageInitialPropsF0108Resource from '../../resources/posts-page-initial-props-f0108'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Corporate Communications Facilitator</title>
          <meta
            property="og:title"
            content="Posts1 - Corporate Communications Facilitator"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths483bfResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialPropsF0108Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
